...
router.get("/filtros", async (req, res) => {
  try {
    ...
    const { status, search, startDate, endDate, category } = req.query;
    ...
    let answersProcessed = answers.map(answer => {
      ...
      const trabajador = answer.responses?.['Nombre del trabajador'] || safeDecrypt(answer.user?.nombre) || "No especificado";

      const rutTrabajador = answer.responses?.['RUT del trabajador'] ||
        answer.responses?.['RUT'] ||
        "No especificado";
      ...
      return {
        ...
        company: empresaUsuario || 'Empresa Desconocida', // Se muestra pero NO se filtra por ella según petición
        categoryData: answer.category || "",
        priority: answer.priority, // Importante para la vista de tickets
        user: {
          nombre: nombreUsuario,
          empresa: empresaUsuario
        },
        responses: answer.responses // Necesario para detalles
      };
    });
    ...
    // 6. Stats de estados (Basados en el universo filtrado por fecha/status/texto)
    const stats = {
    ...
    // Calcular contadores dinámicos sobre el resultado filtrado
    answersProcessed.forEach(a => {
    ...
    console.error("Error crítico en /soporte/filtros:", err);
    res.status(500).json({
      success: false,
